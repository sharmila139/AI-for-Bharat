"""
Grievance Photo Categorization - Training with Synthetic Data
...
from model_training import GrievanceCategoryModel
...
def generate_synthetic_dataset(
    ...
    Args:
        ...
        num_classes: Number of grievance categories
    ...
    for class_idx in range(num_classes):
        for _ in range(num_samples_per_class):
            ...
            if class_idx == 0:  # Roads - grayish with cracks
                img[:, :, 0] *= 0.5  # Gray tone
                img[:, :, 1] *= 0.5
                img[:, :, 2] *= 0.5
                # Add crack-like patterns
                img[::10, :, :] *= 0.3
            elif class_idx == 1:  # Water - bluish
                img[:, :, 0] *= 0.3
                img[:, :, 1] *= 0.6
                img[:, :, 2] *= 1.0  # More blue
            elif class_idx == 2:  # Electricity - yellow/orange (wires)
                img[:, :, 0] *= 0.9
                img[:, :, 1] *= 0.8
                img[:, :, 2] *= 0.4
            elif class_idx == 3:  # Drainage - dark/murky
                img *= 0.4
                img[:, :, 1] *= 0.6  # Slight green tint
            elif class_idx == 4:  # Waste - mixed colors
                img[:, :, 0] *= 0.7
                img[:, :, 1] *= 0.6
                img[:, :, 2] *= 0.5
            elif class_idx == 5:  # Streetlights - bright spots
                img *= 0.3
                # Add bright spots
                img[::20, ::20, :] = 1.0
            elif class_idx == 6:  # Public property - varied
                img[:, :, 0] *= 0.6
                img[:, :, 1] *= 0.7
                img[:, :, 2] *= 0.6
            elif class_idx == 7:  # Health facility - white/clinical
                img *= 0.9
            elif class_idx == 8:  # Education facility - varied colors
                img[:, :, 0] *= 0.7
                img[:, :, 1] *= 0.7
                img[:, :, 2] *= 0.8
...
def train_with_synthetic_data(
    ...
    """
    print("="*60)
    print("Grievance Categorization - Training with Synthetic Data")
    print("="*60)
    print("\nNote: This uses synthetic data for demonstration purposes.")
    print("For production use, train with real grievance images.")
    ...
    # Initialize model
    print("\n1. Building model...")
    model = GrievanceCategoryModel(num_classes=9)
    ...
    # Check if accuracy meets 85% confidence threshold requirement
    if metrics['test_accuracy'] >= 0.85:
        print(f"✓ Model meets 85% confidence threshold requirement")
    else:
        print(f"⚠️  Model accuracy ({metrics['test_accuracy']:.2%}) is below 85% threshold")
        print(f"   Consider training with more data or adjusting hyperparameters")
    
    # Save model
    print("\n5. Saving model...")
    os.makedirs(output_dir, exist_ok=True)
    
    model_path = f'{output_dir}/grievance_classifier.h5'
    model.model.save(model_path)
    print(f"   Keras model saved: {model_path}")
    
    # Save metadata
    metadata = {
        'model_name': 'grievance_classifier',
    ...
    model_size = model.optimize_for_mobile(f'{output_dir}/grievance_classifier_optimized.tflite')
    ...
    print("For production use, retrain with real grievance images.")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Train grievance categorization model with synthetic data'
    )
    parser.add_argument(
        '--samples',
        type=int,
